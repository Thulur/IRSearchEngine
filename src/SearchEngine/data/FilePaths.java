package SearchEngine.data;

/**
 * Created by sebastian on 04.11.2015.
 */
public class FilePaths {
    public static String INDEX_PATH = "data/index.txt";
    public static String DOCINDEX_FILE = "data/docindex.txt";
    public static String POSTINGLIST_PATH = "data/postinglist.txt";
    public static String INDEX_CITATION_PATH = "data/indexcitation.txt";
    public static String POSTINGLIST_CITATION_PATH = "data/postinglistcitation.txt";
    public static String RAW_PARTIAL_PATH = "data/ipgxml/";
    public static String PARTIAL_PATH = "data/partialindices/";
    public static String COMPRESSED_INDEX_PATH = "data/compressed_index.txt";
    public static String COMPRESSED_POSTINGLIST_PATH = "data/compressed_postinglist.txt";
    public static String FILE_IDS_FILE = "data/fileids.txt";
    public static String CACHE_PATH = "data/ipgxml/";
    public static String INDEX_DATA_FILE = "data/indexdata.txt";
}
